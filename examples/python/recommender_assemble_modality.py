...
def main():
    ...
    if not configs:
        ...
        return
    # Sample#1: Assemble modalities - MMTF (video) + Poison-RAG-Plus (text)
    print("\n----------- MMTF (video) + Poison-RAG-Plus (text) -----------")
    ...
    # Sample#2: Assemble modalities - MMTF (audio+video) + Poison-RAG-Plus (text)
    ...
    print(f"- Training set size: {trainDF.shape}, Testing set size: {testDF.shape}")
    print(f"- Available modalities: {list(modalitiesDict.keys())}")
    # Sample#3: Assemble modalities - Popcorn (visual) + MMTF (audio) + Poison-RAG-Plus (text)
