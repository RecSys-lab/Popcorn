...
from movifex.datasets.textaug.text import loadText
from movifex.multimodal.utils import MULTI_VARIANTS
from movifex.datasets.mmtf14k.audio import loadAudio
from movifex.datasets.mmtf14k.visual import loadVisual
...
def prepareModalities(config: dict, train_df: pd.DataFrame, test_df: pd.DataFrame):
    ...
    vis_df, aud_df, txt_df = [
        df[df.itemId.isin(common)].reset_index(drop=True)
        for df in (vis_df, aud_df, txt_df)
    ]
    ...
    merged["all"] = merged.apply(
        lambda r: np.hstack([r.audio, r.visual, r.text]), axis=1
    )
    ...
    train_set = Dataset.from_uir(
        train_df[["user_id", "item_id", "rating"]].values.tolist()
    )
    if VERBOSE:
    ...
    def _im(col):
        return ImageModality(
            features=np.vstack(merged[col]), ids=merged.itemId, normalized=True
        )
    ...
    for tag, param in MULTI_VARIANTS:
        ...
        if tag == "pca":
            ...
            if VERBOSE:
                print(f"✔ PCA {int(ratio*100)} dims = {mat.shape[1]}")
        elif tag == "cca":
            comps = param
            name = f"cca_{comps}"
            half = len(merged["all"][0]) // 2
            big = np.vstack(merged["all"])
